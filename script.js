...
// --- tsParticles Initialization (Corrected: Only Background Particles) ---
if (typeof tsParticles !== 'undefined') {
    tsParticles.load("tsparticles", {
        fpsLimit: 60,
        // particles: defines the main background particles
        particles: {
            number: {
                value: 70, // Number of particles
                density: {
                    enable: true, // Allow density calculation
                    area: 800 // Adjust density based on area
                }
            },
            color: {
                // Colors for the background particles
                value: ["#FFFFFF", "#ADD8E6", "#F0F8FF", "#86efac", "#67e8f9"]
            },
            shape: {
                // Shapes for the background particles (circles and stars)
                type: ["circle", "star"]
            },
            opacity: {
                value: { min: 0.1, max: 0.6 }, // Opacity range
                animation: { // Twinkling effect
                    enable: true,
                    speed: 0.8,
                    minimumValue: 0.1,
                    sync: false,
            ...
            size: {
                value: { min: 0.5, max: 2.5 }, // Size range
                animation: { // Pulsing size effect (Optional, remove if unwanted)
                    enable: true,
                    speed: 4,
                    minimumValue: 0.5,
                    sync: false,
            ...
            links: {
                ...
                color: "random",
                distance: 130,
                enable: true, // Set to true if you want lines, false if not
                opacity: 0.15,
                width: 1,
                warp: false
            },
            collisions: {
                enable: false // Keep collisions off for performance
            },
            move: {
                // How particles move
                direction: "none",
                enable: true,
                outModes: {
                    default: "out" // How they behave when hitting edges
                },
                random: true, // Random movement
                speed: 0.5, // Base speed
                straight: false, // Not moving in straight lines
                noise: { // Adds slight randomness/wobble to movement
                    enable: true,
                    delay: { value: 0.5 },
                    factor: { value: 0.5 }
                }
        ...
        // interactivity: defines hover/click effects
        interactivity: {
            events: {
                onHover: {
            ...
            modes: {
                repulse: { distance: 70, duration: 0.4, speed: 0.6 },
                push: { quantity: 3 } // Number of particles added on click
            }
        },
        // *** NO EMITTERS SECTION ***
        detectRetina: true, // Adjusts for high-DPI screens
        background: {
            opacity: 0 // Makes the tsParticles canvas background transparent
        }
    }).then(container => {
        console.log("tsParticles loaded successfully"); // Confirmation message
    }).catch(error => {
        console.error("Error loading tsParticles:", error); // Error handling
    });
} else {
